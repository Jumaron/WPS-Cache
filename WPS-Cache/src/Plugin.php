...
final class Plugin
{
    ...
    private function toggleWPCache(bool $enable): bool
    {
        ...
        if ($enable) {
            if (preg_match($pattern, $content))
                $new = preg_replace($pattern, "define('WP_CACHE', true);", $content);
            else
                $new = preg_replace('/^<\?php/m', "<?php\r\ndefine('WP_CACHE', true);", $content, 1);
        } else {
            $new = preg_replace("/define\s*\(\s*['\"]WP_CACHE['\"]\s*,\s*(true|false)\s*\)\s*;\s*/i", "", $content);
        }
    ...
    {
        $files = [WP_CONTENT_DIR . '/advanced-cache.php', WP_CONTENT_DIR . '/object-cache.php'];
        foreach ($files as $file) {
            if (file_exists($file) && (str_contains(file_get_contents($file), 'WPS-Cache') || str_contains(file_get_contents($file), 'WPS Cache')))
                @unlink($file);
        }
    }
